...
import { Award, CheckCircle, Sparkles, ExternalLink, QrCode } from 'lucide-react';
import Link from 'next/link';
import { SceneContainer } from './SceneContainer';

interface Scene5VerifiedProps {
  progress: MotionValue<number>;
}

export function Scene5Verified({ progress }: Scene5VerifiedProps) {
  // Laser scanner sweep across QR code
  const laserY = useTransform(progress, [0.88, 0.98], ['-10%', '110%'], { clamp: true });
  const laserOpacity = useTransform(progress, [0.87, 0.91, 0.97, 1.0], [0, 1, 1, 0.8], { clamp: true });
  const badgeScale = useTransform(progress, [0.90, 0.98], [0.85, 1], { clamp: true });

  return (
    <SceneContainer
      progress={progress}
      enterRange={[0.84, 0.89]}
      exitRange={[1, 1]}
      isLast
    ...
    >
      <div className="flex flex-col items-center relative max-w-3xl mx-auto">
        ...
        {/* Top Eyebrow */}
        <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-emerald-100/95 border border-emerald-300 text-emerald-950 text-xs font-extrabold uppercase tracking-wider mb-4 shadow-sm">
          <Award className="w-3.5 h-3.5 text-emerald-600" />
          <span>05. VERIFIED AUTHENTIC HONEY</span>
        </div>
