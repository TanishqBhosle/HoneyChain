...
interface Scene3TraceabilityProps {
  progress: MotionValue<number>;
}

export function Scene3Traceability({ progress }: Scene3TraceabilityProps) {
  // Golden Path SVG draw progress (from 0% offset to full path draw) for Scene 3 (progress 0.40 -> 0.64)
  const pathOffset = useTransform(progress, [0.41, 0.58], [1000, 0], { clamp: true });

  const node1Opacity = useTransform(progress, [0.41, 0.44], [0, 1], { clamp: true });
  const node2Opacity = useTransform(progress, [0.44, 0.47], [0, 1], { clamp: true });
  const node3Opacity = useTransform(progress, [0.47, 0.50], [0, 1], { clamp: true });
  const node4Opacity = useTransform(progress, [0.50, 0.53], [0, 1], { clamp: true });
  const node5Opacity = useTransform(progress, [0.53, 0.56], [0, 1], { clamp: true });
  const node6Opacity = useTransform(progress, [0.56, 0.59], [0, 1], { clamp: true });
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center relative max-w-4xl mx-auto">
        ...
        {/* Top Eyebrow */}
        <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-amber-100/95 border border-amber-300 text-amber-950 text-xs font-extrabold uppercase tracking-wider mb-4 shadow-sm">
          <Route className="w-3.5 h-3.5 text-amber-600" />
          <span>03. THE GOLDEN TRACEABILITY PATH</span>
        </div>

        {/* Primary Heading */}
        <h2 className="text-3xl sm:text-5xl font-black font-heading text-slate-950 tracking-tight leading-tight mb-3 drop-shadow-[0_1px_1px_rgba(255,255,255,0.9)]">
        ...
        <div className="relative w-full aspect-[960/280] max-h-[320px] px-2">
          <svg viewBox="0 0 960 280" className="w-full h-full overflow-visible">
            ...
            <motion.path
              d="M 80 160 C 160 160, 160 90, 240 90 C 320 90, 320 170, 400 170 C 480 170, 480 100, 560 100 C 640 100, 640 180, 720 180 C 800 180, 800 110, 880 110"
              fill="none"
              stroke="url(#traceGradS3)"
              strokeWidth="6"
              strokeLinecap="round"
              strokeDasharray={1000}
              style={{ strokeDashoffset: pathOffset }}
              filter="url(#traceGlowS3)"
            />
          </svg>
